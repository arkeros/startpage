...
var Help = (function () {
    ...
    config.categories.forEach(function (category) {
        ...
        category.commands.forEach(function (command) {
            var favicon = command.favicon || (command.url + '/favicon.ico');
            commandItems += (
                '<li class="command">' +
                '<a href="' + command.url + '">' +
                '<span class="command-key">' + command.key + '</span>' +
                '<img class="command-favicon" src="' + favicon + '" />' +
        ...
        show: function (input) {
            ...
            queries
                .filter(function (query) {
                    var matchesQuery = query[0].indexOf(input) == 0;
            ...
            category.commands.forEach(function (command) {
                ...
                if (isSearch || isPath) {
                    if (qSplitSearch[1] && command.search) {
                        ...
                        var dash = command.dash || "/";
                        redirect = command.url + dash + path;
                    } else {
